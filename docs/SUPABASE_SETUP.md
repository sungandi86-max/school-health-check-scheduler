...
- 앱 설정 화면에서 `localStorage` / `Supabase` 저장 모드를 선택할 수 있습니다.
- 기본값은 `localStorage`이며, 현재 운영 데이터는 이 브라우저에 저장됩니다.
- Supabase 환경변수가 없으면 Supabase 선택은 비활성화됩니다.
- Supabase 환경변수가 있어도 실제 테이블 읽기/쓰기는 아직 연결되지 않았으므로, 현재 버전은 localStorage fallback을 유지합니다.
- 실제 Supabase 저장을 활성화하기 전에는 localStorage 데이터를 백업하고, 마이그레이션 절차를 먼저 검증해야 합니다.
